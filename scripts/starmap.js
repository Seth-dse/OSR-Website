...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const starMaterial = new THREE.PointsMaterial({
    size: 0.6, map: starTexture, vertexColors: true, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
  });
  ...
  Object.values(starSystems).forEach(sys => {
    ...
    let factionColorHex = '#00e5ff';
    if (sys.faction === 'unclaimed') factionColorHex = '#ffab00';
    else if (sys.faction === 'independent') factionColorHex = '#00e676';
    else if (sys.faction === 'vanduul') factionColorHex = '#ff1744';

    const ringGeo = new THREE.RingGeometry(1.6, 1.8, 32);
    const ringMat = new THREE.MeshBasicMaterial({ color: factionColorHex, side: THREE.DoubleSide, transparent: true, opacity: 0.45 });
  ...
  const jumpLineMaterial = new THREE.LineDashedMaterial({
    color: 0x00e5ff, dashSize: 1.5, gapSize: 1, transparent: true, opacity: 0.35, blending: THREE.AdditiveBlending
  });
  const jumpLines = [];
  ...
  Object.values(starSystems).forEach(sys => {
    sys.jumps.forEach(targetId => {
      ...
      const points = [new THREE.Vector3(sys.coordinates.x, sys.coordinates.y, sys.coordinates.z), new THREE.Vector3(target.coordinates.x, target.coordinates.y, target.coordinates.z)];
      const lineGeo = new THREE.BufferGeometry().setFromPoints(points);
      const line = new THREE.Line(lineGeo, jumpLineMaterial.clone());
      line.computeLineDistances();
      scene.add(line);
      jumpLines.push({ line, faction1: sys.faction, faction2: target.faction });
    });
  });

  /* ==========================================
     5. FOCUSED SYSTEM VIEW RENDERER
     ========================================== */
  let activeSystemId = null;
  let systemViewActive = false;
  let localPlanets = [];
  let localOrbitLines = [];
  let localStarGroup = null;
  ...
  function enterSystemView(systemId) {
    ...
    labelElements.forEach(item => { if (item.id !== systemId) item.element.style.opacity = '0'; });
    ...
    sys.planets.forEach(p => {
      ...
      const orbitMat = new THREE.MeshBasicMaterial({ color: 0x00e5ff, transparent: true, opacity: 0.08, side: THREE.DoubleSide });
      ...
      const planetMat = new THREE.MeshStandardMaterial({ color: p.color, roughness: 0.8, metalness: 0.1 });
      const planetMesh = new THREE.Mesh(planetGeo, planetMat);
      planetMesh.position.x = p.distance;
      planetPivot.add(planetMesh);
      localPlanets.push({ pivot: planetPivot, mesh: planetMesh, speed: p.speed, angle: Math.random() * Math.PI * 2, name: p.name, type: p.type });
      const pLabel = document.createElement('div');
      pLabel.className = 'starmap-label';
      pLabel.style.color = '#ffab00'; pLabel.style.borderLeftColor = '#ffab00';
    ...
    Object.keys(starGroups).forEach(id => { if (id !== systemId) starGroups[id].visible = false; });
  ...
  function exitSystemView(resetCamera = true) {
    ...
    labelElements.forEach(item => { item.element.style.opacity = '1'; });
    const planetLabels = labelElements.filter(item => item.isPlanet);
    planetLabels.forEach(item => { if (item.element.parentNode) item.element.parentNode.removeChild(item.element); });
    for (let i = labelElements.length - 1; i >= 0; i--) { if (labelElements[i].isPlanet) labelElements.splice(i, 1); }
    if (localStarGroup) { scene.remove(localStarGroup); localStarGroup = null; }
    localPlanets = []; localOrbitLines = [];
    Object.keys(starGroups).forEach(id => { starGroups[id].visible = true; });
    document.getElementById('hud-panel-right').classList.remove('active');
    updateSystemListUI(null);
    if (resetCamera) tweenCamera({ x: 0, y: 0, z: 0 }, 110);
  }
  ...
  function buildSystemList(filteredList = Object.values(starSystems)) {
    ...
    filteredList.forEach(sys => {
      ...
      li.innerHTML = `<button type="button"><span>${sys.name}</span><span class="system-list-item-faction">${sys.faction.toUpperCase()}</span></button>`;
      li.addEventListener('click', () => enterSystemView(sys.id));
  ...
  function updateSystemListUI(activeId) {
    ...
    items.forEach(item => {
      ...
      if (activeId && item.id === `list-item-${activeId}`) item.classList.add('active');
    });
  }

  async function updateHUDDetails(sys) {
    ...
    // LIVE STAMAP UPDATE: Fetch latest intel for this system
    const descEl = document.getElementById('telemetry-description');
    descEl.innerHTML = `<span style="color:var(--color-text-muted)">${sys.description}</span>`;

    try {
        const response = await fetch('api/intel.php');
        if (response.ok) {
            const intelData = await response.json();
            const sysIntel = intelData.filter(i => i.system === sys.id).sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));
            if (sysIntel.length > 0) {
                const latest = sysIntel[0];
                descEl.innerHTML = `
                    <div style="border-left: 2px solid var(--color-accent-amber); padding-left: 10px; margin-bottom: 10px;">
                        <span style="color:var(--color-accent-amber); font-family:var(--font-mono); font-size:0.65rem;">[LATEST OPERATIONAL INTEL]</span><br>
                        <span style="color:#fff; font-size:0.85rem;">"${latest.content}"</span><br>
                        <span style="color:var(--color-text-muted); font-size:0.6rem;">${latest.id} // ${new Date(latest.timestamp).toLocaleDateString()}</span>
                    </div>
                    <hr style="border:none; border-top: 1px solid rgba(0,229,255,0.1); margin: 10px 0;">
                    <span style="color:var(--color-text-muted)">${sys.description}</span>
                `;
            }
        }
    } catch (e) { console.error("HUD Live Update Error:", e); }
    ...
    sys.jumps.forEach(jumpId => {
      ...
      if (jumpSys) {
        ...
        li.innerHTML = `<button class="jump-link">${jumpSys.name}</button><span class="detail-list-item-type">JUMP POINT</span>`;
        li.querySelector('button').addEventListener('click', () => enterSystemView(jumpId));
    ...
    sys.planets.forEach(p => {
      const li = document.createElement('li');
      li.className = 'detail-list-item';
      li.innerHTML = `<span class="detail-list-item-name">${p.name}</span><span class="detail-list-item-type">${p.type}</span>`;
    ...
    if (sys.stations && sys.stations.length > 0) {
      sys.stations.forEach(s => {
        const li = document.createElement('li');
        li.className = 'detail-list-item';
        li.innerHTML = `<span class="detail-list-item-name">${s.name}</span><span class="detail-list-item-type">${s.type}</span>`;
  ...
  function applyFactionFilters() {
    const list = Object.values(starSystems).filter(sys => activeFactionFilter === 'all' || sys.faction === activeFactionFilter);
    ...
    Object.values(starSystems).forEach(sys => {
      ...
      if (group) group.traverse(child => { if (child.material) { child.material.transparent = true; child.material.opacity = matchesFilter ? (child.isSprite ? 1.0 : 0.6) : 0.15; } });
      if (label && label.element) label.element.style.opacity = matchesFilter ? '1' : '0.2';
    });
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    if (!query) { applyFactionFilters(); return; }
    const filtered = Object.values(starSystems).filter(sys => sys.name.toLowerCase().includes(query) || sys.planets.some(p => p.name.toLowerCase().includes(query)));
    buildSystemList(filtered);
    const exactMatch = Object.values(starSystems).find(sys => sys.name.toLowerCase() === query);
    if (exactMatch) enterSystemView(exactMatch.id);
  });

  returnBtn.addEventListener('click', () => { searchInput.value = ''; exitSystemView(); applyFactionFilters(); });
  ...
  window.addEventListener('click', () => {
    ...
    const meshes = Object.values(starGroups).map(g => g.children[0]);
    const intersects = raycaster.intersectObjects(meshes);
    if (intersects.length > 0) enterSystemView(intersects[0].object.parent.userData.id);
  });

  let cameraTargetPos = null, controlsTargetLook = null, tweenProgress = 1;
  function tweenCamera(lookAtPos, zoomDistance) {
    controlsTargetLook = new THREE.Vector3(lookAtPos.x, lookAtPos.y, lookAtPos.z);
    cameraTargetPos = new THREE.Vector3(lookAtPos.x + 0.1, lookAtPos.y + zoomDistance * 0.7, lookAtPos.z + zoomDistance * 0.7);
    tweenProgress = 0;
  }

  /* ==========================================
     8. SIGNAL INTELLIGENCE SYSTEM
     ========================================== */
  const intelReportBtn = document.getElementById('intel-report-btn');
  ...
  if (intelReportBtn) intelReportBtn.addEventListener('click', openReportModal);
  if (telemetryReportBtn) telemetryReportBtn.addEventListener('click', openReportModal);
  if (modalClose) modalClose.addEventListener('click', () => intelModal.classList.remove('active'));
  window.addEventListener('click', (e) => { if (e.target === intelModal) intelModal.classList.remove('active'); });
  ...
  if (intelForm) {
    intelForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('api/intel.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newReport) });
        if (!response.ok) throw new Error('Failed to save intel');
        intelModal.classList.remove('active');
        intelForm.reset();
        initIntelFeed();
        if (activeSystemId) updateHUDDetails(starSystems[activeSystemId]); // LIVE UPDATE HUD
        alert("Intelligence Data Transmitted Successfully.");
      } catch (error) { console.error('Error saving report:', error); alert("TRANSMISSION FAILED: Link to Secure Database Interrupted."); }
    });
  }
  ...
  async function initIntelFeed() {
    ...
    try {
      ...
      intelData.forEach(item => {
        ...
        entry.innerHTML = `<div class="intel-header"><span class="intel-id">${item.id}</span><span class="intel-loc">${item.system.toUpperCase()} // ${item.location}</span><span class="intel-time">${timeStr}</span></div><div class="intel-body"><span class="intel-severity severity-${item.severity.toLowerCase()}">[${item.severity}]</span><span class="intel-type">${item.type}:</span><span class="intel-content">${item.content}</span></div>`;
        entry.addEventListener('click', () => { if (starSystems[item.system]) enterSystemView(item.system); });
        feedEl.appendChild(entry);
      });
      document.getElementById('intel-status').textContent = 'SIGNAL STABLE // ' + intelData.length + ' REPORTS';
    } catch (error) { console.error('Error loading intel feed:', error); feedEl.innerHTML = '<div class="intel-entry severity-critical">SIGNAL INTERFERENCE DETECTED // LINK FAILED</div>'; document.getElementById('intel-status').textContent = 'LINK OFFLINE'; }
  }

  /* ==========================================
     9. ANIMATION LOOP & RENDERER
     ========================================== */
  buildSystemList();
  initIntelFeed();
  ...
  function animate() {
    ...
    if (tweenProgress < 1) { tweenProgress += delta * 1.5; if (tweenProgress > 1) tweenProgress = 1; camera.position.lerp(cameraTargetPos, tweenProgress); controls.target.lerp(controlsTargetLook, tweenProgress); }
    jumpLines.forEach(item => item.line.material.dashOffset = -time * 1.2);
    if (systemViewActive && localStarGroup) localPlanets.forEach(p => { p.angle += p.speed; p.pivot.rotation.y = p.angle; });
    controls.update();
    renderer.render(scene, camera);
    const tempV = new THREE.Vector3();
    labelElements.forEach(item => {
      if (systemViewActive && !item.isPlanet && item.id !== activeSystemId) { item.element.style.opacity = '0'; return; }
      if (item.isPlanet) item.position.getWorldPosition(tempV); else tempV.copy(item.position);
      tempV.project(camera);
      const rect = renderer.domElement.getBoundingClientRect();
      const x = (tempV.x * 0.5 + 0.5) * rect.width, y = (-(tempV.y * 0.5) + 0.5) * rect.height;
      if (tempV.z > 1) item.element.style.display = 'none'; else { item.element.style.display = 'block'; item.element.style.left = `${x}px`; item.element.style.top = `${y}px`; }
    });
    if (!systemViewActive) {
      raycaster.setFromCamera(mouse, camera);
      ...
      if (intersects.length > 0) {
        const hoveredMesh = intersects[0].object, parentGrp = hoveredMesh.parent, sysId = parentGrp.userData.id;
        if (hoveredStarId !== sysId) {
          if (hoveredStarId && starGroups[hoveredStarId]) { starGroups[hoveredStarId].scale.set(1, 1, 1); const lbl = labelElements.find(l => l.id === hoveredStarId); if (lbl) lbl.element.style.transform = 'translate(-50%, -100%) scale(1)'; }
          hoveredStarId = sysId; parentGrp.scale.set(1.25, 1.25, 1.25); const lbl = labelElements.find(l => l.id === sysId); if (lbl) lbl.element.style.transform = 'translate(-50%, -100%) scale(1.15)';
        }
      } else if (hoveredStarId && starGroups[hoveredStarId]) { starGroups[hoveredStarId].scale.set(1, 1, 1); const lbl = labelElements.find(l => l.id === hoveredStarId); if (lbl) lbl.element.style.transform = 'translate(-50%, -100%) scale(1)'; hoveredStarId = null; }
    }
  }
  animate();
  window.addEventListener('resize', () => { const newWidth = canvasContainer.clientWidth, newHeight = canvasContainer.clientHeight; camera.aspect = newWidth / newHeight; camera.updateProjectionMatrix(); renderer.setSize(newWidth, newHeight); });
});
